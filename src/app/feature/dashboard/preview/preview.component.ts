...
import { NewExpenseService } from '../../expense/service/new-expense.service';
...
export class PreviewComponent {
  ...
  loadExpenseRequestPreviewData() {
    if (!this.loadData) {
      ...
      this.newExpenseService.getRequestorInfo(body).subscribe((data: any) => {
  ...
  goBack() {
    ...
    if (this.mode == 'approval') {
      this.router.navigate(['/dashboard/approval-dashboard']);
    }
    if (this.mode == 'finance-approval') {
      this.router.navigate(['/finance/settlement']);
    }
  }
